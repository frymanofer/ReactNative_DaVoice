...
#### Quick start: using the remote wake word in this sample (and how to confirm it worked)

This sample is **already wired to a live CDN**, so the remote download runs on every launch with no setup. The detail that makes it easy to verify is that the bundled model and the remote model listen for **different wake phrases**:

| Model | Where it comes from | Wake phrase you say |
|---|---|---|
| **Bundled** | `assets/models/hey_coach_model_28_22012026b.dm`, shipped inside the app | **"hey coach"** |
| **Remote** | downloaded at runtime from `WAKEWORD_MODEL_CDN_BASE_URL` | **"hey look deep"** |

They are deliberately two different wake words so you can hear which model is running:

- The app wakes on **"hey look deep"** (and *not* on "hey coach") → the remote model was downloaded and is live. The replacement worked.
- The app still wakes on **"hey coach"** → it is running the bundled model (no network, the download failed, or you cleared the downloaded copy).

> The file on the CDN is named `hey_coach_model_28_22012026b.dm` for build-pipeline reasons, but its payload is the "hey look deep" model. The file **name** never changes between versions; only its contents (and its `.sha256`) do.

##### Steps

1. Build and run on a device with network access: `npx react-native run-android` or `npx react-native run-ios`.
2. On the first launch after install, tail the JS console and filter for `[WakewordModelUpdate]` / `[WakewordFlow]`:
   ```bash
   npx react-native log-android    # or: npx react-native log-ios
   ```
   Expect `RESULT updated` on the first run (`RESULT up_to_date` on later runs), then
   `getActiveWakewordModelPath: using DOWNLOADED model /.../wakeword_models/<sha256 prefix>/hey_coach_model_28_22012026b.dm`.
3. Say **"hey look deep"** — wake-word detection fires. Saying "hey coach" now does nothing.
4. To A/B against the bundled model, clear the downloaded copy and relaunch **with networking off**:
   ```bash
   # Android (debug build, package com.exampleapp)
   adb shell run-as com.exampleapp rm -rf files/wakeword_models
   # iOS: delete and reinstall the app, then launch in Airplane Mode
   ```
   Now "hey coach" works and "hey look deep" does not — that is the bundled model. Turn networking back on and relaunch to pull the remote model again.
5. The top-right **menu → "Check for wake word update"** runs the same check on demand. When the app is idle it then offers **"Reload now"**, which hot-swaps the model on the running instance without an app restart.

##### Pointing it at your own CDN

Edit `WAKEWORD_MODEL_CDN_BASE_URL` in [src/wakeword/modelUpdater.ts](./src/wakeword/modelUpdater.ts) and upload the two files described below. If the model you host there is a real "hey coach" model, then the bundled and remote wake phrases are both "hey coach" and the spoken A/B check above no longer applies — use the `[WakewordModelUpdate]` logs and the `sha256` recorded in `wakeword_models/manifest.json` to tell which model is active.
...
| iOS | Yes (react-native-wakeword **1.1.144+**) | 1.1.144 adds an early return in `KeyWordDetection.xcframework` for an absolute path that exists on disk, matching Android. On **1.1.143 and older** the framework resolved every model string as a main-bundle asset (`copyAssetToDocumentsDirectory`) and an absolute path failed with `Failed to copy asset`. This app pins **1.1.144**, so downloaded models take effect on iOS. |

This app still contains the pre-1.1.144 safety net: if the native instance ever rejects a downloaded-model path, `addInstanceMulti` / `reloadWakewordModel` in [src/wakeword/index.ts](./src/wakeword/index.ts) log a warning and fall back to the bundled model, and "Reload now" then shows "Could not load the new model" instead of asking for a restart. With 1.1.144 that path is not expected to trigger.
